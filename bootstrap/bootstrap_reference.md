...
작성일: 2026-07-10  
상태: Draft vNext Reference  
범위: AI Ops bootstrap 상세 선택지, 질문 팩, 예시

## 1. 목적

이 문서는 `bootstrap_runbook.md`의 상세 reference다.
...
- Fast Track 질문은 운영 구성에 필요한 맥락 확인까지만 한다.
- AI Ops Agent는 사용자 불편, 제품 아이디어, MVP 범위, 기능 후보를 깊게 발굴하지 않는다.
- 제품 탐색이 필요하면 Apply 이후 첫 PM Agent / Direction Role 명령 후보로 넘긴다.
...
Fast Track에서 묻지 않는 질문:

```text
- 최근 겪은 불편은 무엇인가요?
- 어떤 제품 아이디어가 좋을까요?
- 첫 MVP 기능은 무엇으로 할까요?
- 대상 사용자와 기능 범위를 자세히 정해볼까요?
```

위 질문은 Bootstrap 질문이 아니라 PM Agent / Direction Role의 제품 탐색 질문이다. AI Ops Agent는 해당 질문을 Apply 이후 첫 세션 명령 후보로만 제안한다.
...
- Agent 온보딩용 최소 Knowledge Wiki를 만듭니다.
...
| `blank_slate_discovery` | 뭘 만들지부터 정해야 하는 백지 탐색 | 제품 Task 없이 첫 PM/Direction 세션 시작 기준을 구성한다 | 아이템 발굴, 리서치, 사업 주제 탐색 |
...
Fast Track의 `.ai_knowledge/` 생성은 `knowledge_mode: minimal`을 사용자가 승인한 경우에만 포함한다. `.ai_knowledge/`는 source of truth가 아니라 Agent 온보딩용 LLM Wiki다.
...
- 제품 Task를 만들지 않는다.
- Bootstrap 중에는 문제 정의와 탐색 질문을 실제로 진행하지 않는다.
- Direction Role 중심의 첫 세션 명령 후보를 제안한다.
...
제품 아이디어 탐색은 여기서 이어서 진행하지 않고, Apply 이후 PM Agent / Direction Role 첫 세션으로 넘기겠습니다.
```
...
- blank_slate_discovery: 무엇을 할지부터 찾아야 합니다. Bootstrap에서는 첫 PM/Direction 세션 시작 기준만 구성합니다.
