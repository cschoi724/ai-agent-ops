...
| `agent_rename_plan.schema.json` | `aiops agent rename --check --json` JSON | 안정 ID로 고정한 Agent 이름 변경, 현재 Task write set, hash·worktree·canonical 사전 조건 |
| `agent_rename_receipt.schema.json` | `aiops agent rename --apply --json` JSON | Registry·현재 Task에 적용된 이름 변경과 보존한 역사 참조 감사 결과 |
| `agent_identity_migration_plan.schema.json` | `aiops agent migrate-identities --check --json` JSON | legacy Registry·Task의 안정 ID 이관 계획과 원자적 write set |
| `agent_identity_migration_receipt.schema.json` | `aiops agent migrate-identities --apply --json` JSON | 실제 ID 이관 결과와 검증·rollback 가능 action 기록 |
...
Agent identity migration 단계 이후 `aiops agent migrate-identities`와 `aiops agent rename`은 기본 read-only plan과 명시적 apply를 분리한다. Git common-dir lock, canonical cache, linked worktree cleanliness, content hash를 확인한 뒤 Registry와 active/backlog Task를 원자적으로 갱신하며 실패 시 파일 내용과 permission을 복원한다. archive와 역사 문서는 변경하지 않고 적용 receipt만 Git-ignored runtime cache에 남긴다.
