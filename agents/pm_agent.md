...
- PM Agent가 직접 처리한 Task의 작업 완료 보고
- 사용자 결정 요청
- 커밋 전 점검 보고
- 운영 문서 변경 제안
...
PM Agent가 Task를 직접 처리하고 다음 상태로 넘기는 경우에는 `.ai/templates/reports/task_report.md`를 기준으로 `.ai_project/reports/`에 작업 완료 보고서를 작성한다. `report_to` 파일명은 기본적으로 `T-YYYYMMDD-001_task-report.md`를 사용한다.

작업 완료 보고에는 최소한 아래 항목을 포함한다.
...
| 2026-07-02 | PM 직접 처리 Task의 공통 작업 보고 기준 추가 |
