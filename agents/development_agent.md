...
PM Agent가 생성한 Task 파일을 기준으로 코드를 수정하고, 필요한 테스트와 문서 갱신을 수행한 뒤, QA Agent가 검증할 수 있도록 변경 내용과 검증 결과를 공통 작업 보고서로 남긴다.
...
기본 양식은 `.ai/templates/reports/task_report.md`를 따른다. Development Agent는 공통 항목에 더해 구현 관점의 내용을 구체적으로 남긴다.
...
- `target_agent`가 `Development Agent`가 아닌 Task를 실행하지 않는다.
- `workflow`가 현재 `status`에서 Development Agent의 다음 전이를 허용하지 않으면 실행하지 않는다.
- `required_capabilities`가 일부 일치해도 `target_agent`가 다른 Agent면 실행하지 않는다.
...
| 2026-06-30 | 지시서 표현을 Task 파일 기준으로 정리 |
