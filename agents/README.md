...
| Agent | 핵심 책임 | 주요 산출물 |
|---|---|---|
| PM Agent | 방향 정리, Task 관리, 지시서 작성, 승인 게이트 관리 | 작업 지시서, Task Board, 결정 필요 항목 |
| Development Agent | 승인된 범위의 구현, 테스트, 개발 보고 | 코드 변경, 테스트 결과, 개발 완료 보고 |
| QA Agent | 구현 결과 검증, 리스크 분석, 재작업 지시 | QA 보고서, 재작업 지시서, 잔여 리스크 |
