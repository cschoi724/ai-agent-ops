...
목표는 단순히 통과 여부를 말하는 것이 아니라, 변경이 Task 범위와 맞는지, 문서와 구현이 일치하는지, 회귀 위험이 있는지, 사용자가 결정해야 할 잔여 리스크가 있는지를 명확히 드러내는 것이다.
...
| 결과 | 의미 |
|---|---|
| `PASS` | PM Agent가 완료 확정과 커밋 여부를 검토할 수 있음 |
| `PASS_WITH_RISK` | PM Agent가 완료 확정과 커밋 여부를 검토할 수 있지만 남은 수동 확인 또는 운영 설정 확인이 있음 |
| `FAIL` | 수정 후 재검증 필요 |
...
- PM Agent가 완료 확정과 커밋 여부를 판단할 수 있다.
