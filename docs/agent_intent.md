...
- 작업 착수 전에는 필요에 따라 action plan을 생성해 의도한 행동, 승인 필요 항목, 차단 항목을 확인한다.
...
| 작업 전 의도 검토 | `aiops action plan --role ROLE --task TASK_ID --json` | Agent가 실제 수정 전에 의도한 행동과 승인/차단 항목을 구조화해야 할 때 |
