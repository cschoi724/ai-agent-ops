...
| `.ai_project/` | 프로젝트별 작업 보드, handoff, 보고서, Agent 운영 결정, source of truth 매핑 | 현재 프로젝트 저장소에 포함 | 프로젝트 운영 중 갱신 가능 |
...
## 5. PM Agent의 `.ai_project/` 생성 권한

PM Agent는 사용자가 프로젝트에 AI Agent 운영을 적용하라고 명시한 경우 `.ai_project/`를 생성할 수 있다.

생성 조건:

- `.ai/`가 존재한다.
- 사용자가 현재 프로젝트에 AI Agent 운영 초기화를 요청했다.
- PM Agent가 생성할 파일 목록을 먼저 제시했다.
- 사용자가 생성 범위를 승인했다.

PM Agent가 생성할 수 있는 기본 파일:
...
PM Agent는 `.ai_project/`를 생성하더라도 `.ai/` 운영 문서는 수정하지 않는다.

## 6. `.ai_project/` 수정 권한

| 영역 | 기본 수정 주체 | 승인 필요 여부 |
|---|---|---|
| `.ai_project/task_board.md` | PM Agent | 일반 상태 갱신은 가능, 우선순위 변경은 사용자 확인 권장 |
...
| `.ai_project/handoffs/` | PM/Dev/QA Agent | 각 Agent가 담당 handoff 작성 가능 |
