...
8. `.ai/bootstrap/cold_start_prompt.md`
9. `.ai/bootstrap/install_runbook.md`
10. `.ai/bootstrap/project_bootstrap_policy.md`
11. `.ai/bootstrap/bootstrap_runbook.md`
12. 현재 템플릿 버전은 `.ai/VERSION`
13. 버전별 변경 이력은 `.ai/CHANGELOG.md`
14. `.ai/runtime/workflow.md`
15. `.ai/policies/document_governance.md`
16. `.ai/policies/project_workspace.md`
17. `.ai/models/agent_registry.md`
18. `.ai/models/capabilities.md`
19. `.ai/policies/commit_policy.md`
20. `.ai/policies/branch_pr_policy.md`
21. `.ai/runtime/task_queue.md`
22. 운영 점검 Role이면 `.ai/agents/ai_ops_agent.md`
23. 작업 유형에 맞는 `.ai/workflows/*.md`
24. 프로젝트가 초기화되어 있으면 `.ai_project/operating_model.md`, `.ai_project/current_context.md`, `.ai_project/tasks/` 문서
...
프로젝트에서 AI Ops 구성을 시작할 때는 install과 bootstrap을 분리해서 진행한다.
...
Install은 `.ai/` 운영체계 템플릿을 설치하는 단계다. Bootstrap은 `.ai_project/`에 프로젝트별 운영 구성을 만드는 단계다.

단, 대상 프로젝트에 아직 `.ai/`가 없고 새 Codex 세션이 ai-agent-ops 원본 경로를 모르는 최초 1회 상황에서는 `.ai/bootstrap/cold_start_prompt.md`의 문장처럼 원본 경로를 함께 알려준다.

실제 프로젝트에 `.ai/` 운영체계 템플릿이 없으면 먼저 `.ai/bootstrap/install_runbook.md` 기준으로 설치 절차를 진행한다. Install은 `.ai/`만 준비하고, `.ai_project/` 구성은 bootstrap에서 별도로 승인받아 진행한다.
...
| 2026-07-10 | `.ai/` install runbook 문서 참조 추가 |
| 2026-07-10 | `.ai/` 미설치 프로젝트용 cold start prompt 추가 |
