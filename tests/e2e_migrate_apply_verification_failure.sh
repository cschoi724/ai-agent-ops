...
grep -q 'missing: .ai_knowledge/README.md' /tmp/aiops-e2e-migrate-verify-fail.out || {
  printf '%s\n' "migrate apply did not surface migration verification failure" >&2
  exit 1
}
