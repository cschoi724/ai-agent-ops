...
| QA 기준 | `QA_CHECKLIST.md` | 테스트 코드, QA 보고 | QA Agent가 리스크 분류 |
| 미확정 질문 | `PENDING_QUESTIONS.md` | 사용자 대화, 이슈 | 사용자 결정 후 관련 문서 반영 |
| 미구현/보류 기능 | `UNIMPLEMENTED_FEATURES.md` | 구현 계획, 이슈 | 우선순위는 PM Agent가 확인 |
...
| 목적 | 명령 또는 절차 | 실행 주체 |
|---|---|---|
| 정적 검사 | {{LINT_COMMAND}} | Development Agent |
| 테스트 | {{TEST_COMMAND}} | Development Agent / QA Agent |
| 빌드 | {{BUILD_COMMAND}} | Development Agent |
| 수동 QA | `QA_CHECKLIST.md` 기준 | QA Agent |
